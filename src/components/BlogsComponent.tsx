...
import { BookOpen, Filter, ExternalLink, Calendar, Search, RefreshCw } from 'lucide-react';
...
const BlogsComponent: React.FC = () => {
  ...
  }, [user?.id, user?.auth_token, selectedCategory]);
  ...
  const fetchBlogsAndCategories = async () => {
    ...
    try {
      ...
      if (data.errors) {
        setError('Failed to fetch blogs and categories');
        return;
      }

      setBlogs(data.data?.getSections || []);
      
      // Only fetch categories once (when selectedCategory is 'All')
      if (selectedCategory === 'All' && data.data?.getSectionClasses) {
        setCategories(data.data.getSectionClasses);
      }
    } catch (error) {
  ...
  const handleBlogClick = (blog: Blog) => {
    if (blog.url) {
      window.open('https://subspace.money/blog/' + blog.url);
    }
  };
            ...
            <div 
              ...
              <div className="flex items-center space-x-3 w-max px-1">
                <button
                  ...
                  className={`flex-shrink-0 whitespace-nowrap px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${
