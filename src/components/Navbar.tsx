...
import {useUnreadChatsCount} from '../hooks/chat/useUnreadChatsCount';
  ...
  const {data: unreadChatsCount = 0} = useUnreadChatsCount({userId: user?.id, authToken: user?.auth_token});
	...
	const handleSearchSubmit = (e: React.FormEvent) => {
		...
		if (searchQuery.trim()) {
			...
			if (suggestion) {
				navigate(`/search?title=${suggestion.title}&suggestionId=${suggestion.id}&index=${suggestion.index}`);
			}
		}
	};
      ...
      <div className="w-full px-3 md:px-4 lg:px-6">
        ...
        <div className="flex items-center justify-between gap-2 md:gap-3 h-16">
          ...
          <Link to="/" className="flex items-center flex-shrink-0">
            ...
            <img src="/favicon.svg" alt="Subspace" className="h-8 w-8 lg:hidden" />
            {/* Desktop logo - full logo with name */}
            <img src="/subspace-hor_002.svg" alt="Subspace" className="h-8 w-auto hidden lg:block" />
          </Link>
        ...
        <div className="md:hidden bg-dark-500 border-t border-gray-800 absolute top-full left-0 right-0 shadow-lg">
          <div className="container mx-auto px-4 py-3">
            <nav className="flex flex-col space-y-3 items-start">
              {isAuthenticated ? (
                <>
                  <Link to="/chat" className="w-full text-left text-gray-300 hover:text-white transition-colors py-2 flex items-center gap-2">
                  ...
                  <Link to="/wallet" className="w-full text-left text-gray-300 hover:text-white transition-colors py-2">
                    {t('nav.wallet')}
                  </Link>
                  <Link
                    to="/manage"
