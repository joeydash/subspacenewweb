...
import { ArrowRight, CircleAlert as AlertCircle, CircleCheck as CheckCircle, X } from 'lucide-react';
import { useAuthStore, initiatePhoneAuth, verifyOTP } from '../store/authStore';
import { useLanguageStore } from '../store/languageStore';
import { useNetworkStatus } from '../hooks/useNetworkStatus';
import {saveQuickAddress} from '../utils/saveQuickAddress';
...
const LoginModal = ({ isOpen, onClose, onSuccess }: LoginModalProps) => {
  ...
  const fetchUserInfo = async (id: string, authToken: string) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 xs:p-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-md bg-dark-600 rounded-xl sm:rounded-2xl shadow-2xl animate-fade-in max-h-[95vh] overflow-y-auto">
        ...
        <div className="absolute inset-0 overflow-hidden rounded-xl sm:rounded-2xl pointer-events-none">
          ...
          <div className="absolute top-20 right-10 w-16 h-16 bg-purple-500/20 rounded-full blur-xl animate-pulse delay-1000"></div>
          <div className="absolute bottom-20 left-1/4 w-24 h-24 bg-blue-500/10 rounded-full blur-xl animate-pulse delay-500"></div>
        </div>
        ...
        <button
          ...
          onClick={handleClose}
          disabled={isLoading}
          className="absolute top-3 right-3 sm:top-4 sm:right-4 text-gray-400 hover:text-white transition-colors p-1.5 sm:p-2 rounded-lg hover:bg-dark-500 disabled:opacity-50 z-50 cursor-pointer"
        >
          <X className="h-4 w-4 sm:h-5 sm:w-5" />
        </button>

        {/* Content */}
        <div className="relative z-10 p-4 xs:p-5 sm:p-8">
          {/* Logo */}
          <div className="text-center mb-4 sm:mb-6">
            <div className="flex justify-center mb-2 sm:mb-3">
              <img src="/subspace-hor_002.svg" alt="Subspace" className="h-6 xs:h-7 sm:h-8 w-auto" />
            </div>
              ...
              <form onSubmit={handlePhoneSubmit}>
                <div className="mb-4 sm:mb-6">
                  ...
                  <div className="relative group">
                    <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/20 to-purple-500/20 rounded-lg sm:rounded-xl blur opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                    <input
                      type="tel"
                      id="modal-phone"
                      value={phone}
                  ...
                  <p className="text-gray-400 text-xs xs:text-sm mt-2 sm:mt-3 flex items-center gap-1.5 xs:gap-2">
                    <svg className="w-3.5 h-3.5 xs:w-4 xs:h-4 text-indigo-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth={2}
                ...
                {error && (
                  <div className="mb-4 sm:mb-6 p-2.5 xs:p-3 bg-red-500/10 border border-red-500/20 text-red-300 rounded-lg sm:rounded-xl flex items-start backdrop-blur-sm animate-fade-in">
                    <AlertCircle className="h-4 w-4 xs:h-5 xs:w-5 mr-2 xs:mr-3 mt-0.5 text-red-400 flex-shrink-0" />
              ...
              <form onSubmit={handleOtpSubmit}>
                <div className="mb-4 sm:mb-6">
                  ...
                  <p className="text-gray-300 text-xs xs:text-sm mb-4 sm:mb-6 flex items-start gap-1.5 xs:gap-2">
                    <svg className="w-3.5 h-3.5 xs:w-4 xs:h-4 text-indigo-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth={2}
            ...
            {step === AuthStep.VERIFIED && (
              <div className="text-center relative z-10 animate-fade-in">
                <div className="flex justify-center mb-3 sm:mb-4">
                  <div className="bg-green-500/20 backdrop-blur-sm p-3 sm:p-4 rounded-2xl border border-green-500/30 animate-pulse">
                    <CheckCircle className="h-8 w-8 xs:h-9 xs:w-9 sm:h-10 sm:w-10 text-green-400" />
                  </div>
                </div>
                <h2 className="text-xl xs:text-2xl font-bold mb-2 sm:mb-3 text-white">{t('auth.success.title')}</h2>
                <p className="text-gray-300 mb-4 sm:mb-6 text-sm xs:text-base">{t('auth.success.subtitle')}</p>
              </div>
            )}
          </div>
        </div>
