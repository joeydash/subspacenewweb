...
import { ArrowLeft, ShoppingCart, ExternalLink, Smartphone, ChevronDown, ChevronUp, Info, X, Star, Users, Clock, Package, Plus, Minus, Trash2, UserPlus } from 'lucide-react';
import { useAuthStore } from '../store/authStore';
import { useLanguageStore } from '../store/languageStore';
import PlanDetailsModal from '../components/PlanDetailsModal.tsx';
...
import {useRentProducts} from '../hooks/rent/useRentProducts';

import {useCart, CART_BASE_KEY} from '../hooks/cart/useCart';
import {useAddToCartMutation} from '../hooks/cart/useAddToCart';
import {useRemoveFromCartMutation} from '../hooks/cart/useRemoveFromCart';
...
const FlexiPaySection = ({ serviceDetails, isAuthenticated, onLoginRequired }: FlexiPaySectionProps) => {
  ...
  const handleFlexiPayPurchase = async () => {
    if (!isFlexiPayValid() || !user?.id || !user?.auth_token || !serviceDetails) return;
  ...
  if (!serviceDetails.flexipay) return null;

  return (
    <div className="mb-6 bg-dark-400 rounded-lg p-4 border border-gray-700/50 hover:border-blue-500/30 transition-all">
      {/* Header */}
      <div className="flex items-center gap-3 mb-4 pb-3 border-b border-gray-700/50">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h3 className="text-base font-bold text-white">FlexiPay</h3>
            <span className="bg-emerald-500/10 text-emerald-400 text-xs font-semibold px-2 py-0.5 rounded border border-emerald-500/30">
              {serviceDetails.flexipay_discount}% OFF
            </span>
          </div>
          <p className="text-xs text-gray-400 mt-0.5">
            Choose any amount between ₹{serviceDetails.flexipay_min} - ₹{serviceDetails.flexipay_max}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="relative">
          <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 text-lg font-bold">₹</div>
          <input
            type="number"
            value={flexiPayAmount}
            onChange={(e) => setFlexiPayAmount(e.target.value)}
            placeholder="0"
            className="w-full bg-dark-300 text-white text-2xl font-bold pl-10 pr-4 py-3 rounded-lg border border-gray-600 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all"
            ...
            return amounts.map((amount, index) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all ${
                  flexiPayAmount === amount.toString()
        ...
        {flexiPayAmount && parseFloat(flexiPayAmount) > 0 && isFlexiPayValid() && (
          <div className="flex items-center justify-between bg-dark-300 rounded-lg px-4 py-3">
            <span className="text-sm text-gray-400">You pay after discount</span>
        ...
        <button
          ...
          className={`w-full py-2.5 rounded-lg font-semibold text-sm transition-all flex items-center justify-center gap-2 ${
            isFlexiPayValid() && !isProcessingFlexiPay
...
const ProductPlansSection = ({ serviceDetails, isAuthenticated, onLoginRequired, onPlanInfoClick }: ProductPlansProps) => {
  ...
  const {data: cartData, isLoading: isCartDataLoading} = useCart(user);
          ...
          >
            ...
            <div className="bg-gradient-to-r from-dark-300/50 to-dark-400/50 px-3 py-2.5 flex items-center justify-between border-b border-gray-700/30">
              ...
              <button
                ...
                className="w-7 h-7 rounded-[50%] bg-gray-700/60 hover:bg-gray-600 text-gray-300 hover:text-white rounded flex items-center justify-center transition-colors"
              >
            ...
            <div className="p-4">
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                {plan.duration && plan.duration_type ? (
                  ...
                  <div className="flex items-center gap-1.5 px-2 py-1 rounded text-xs text-gray-500">
                    <Clock className="h-3.5 w-3.5" />
                    <span>No expiry</span>
                  </div>
                )}
...
const ProductDetailPage = () => {
  ...
  const {data, isLoading, error: serviceDetailsError} = useServiceDetails({user, serviceId: id});
  ...
  const handleGroupClick = (distinctPlan: DistinctPlan) => {
    if (distinctPlan.whatsub_plans) {
      navigate('/groups/' + distinctPlan.whatsub_plans.id);
    }
  };

  const handleRentConfirm = (startTime: string, endTime: string) => {
    if (selectedProduct) {
      navigate(`/rental-checkout/${selectedProduct.id}?product_location_id=${selectedProduct.product_location_id}`, {
        state: {
          startTime,
          endTime
        }
  ...
  useEffect(() => {
    ...
    if (section === 'offline_products' && offlineProductsRef.current && !isLoading) {
      setTimeout(() => {
    ...
    <div className="page-container pt-24">
        ...
        <div className="h-48 md:h-64 relative">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 md:p-6">
            <div className="flex items-end gap-3 md:gap-6">
              <div className="w-16 h-16 md:w-24 md:h-24 rounded-xl overflow-hidden flex-shrink-0 bg-white p-2 shadow-lg">
                <img
      ...
      {serviceDetails.flexipay_vendor_instructions && (
        <div className="mb-8">
          <button
            ...
            className="flex items-center justify-between w-full text-left group py-3 px-4 rounded-lg hover:bg-dark-500/50 transition-colors"
          >
      ...
      {(serviceDetails?.whatsub_plans.length > 0 || !!serviceDetails.flexipay) && (
        <div>
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <div>
              <h3 className="text-lg md:text-2xl font-bold">{t('product.availablePlans')}</h3>
              <p className="text-xs md:text-sm text-gray-400">{t('product.availablePlansSubtitle')}</p>
            </div>
            <div className="text-xs md:text-sm text-gray-400">
