...
import { ArrowLeft, CreditCard, Wallet, Tag, AlertCircle, CheckCircle, Clock, Users, Shield, Info, ChevronDown } from 'lucide-react';
...
const CheckoutPage = () => {
  ...
  // Get group details from location state if available
  const stateGroupDetails = location.state?.groupDetails;
  const stateGroupMembers = location.state?.groupMembers;
  ...
  useEffect(() => {
    if (user?.id && user?.auth_token) {
      ...
      // If we have group details from state, use them
      if (stateGroupDetails) {
        setGroupDetails(stateGroupDetails);
      } 
      // Otherwise fetch from API if we have roomId
      else if (roomId) {
        fetchGroupDetails();
      }
      
      // If we have group members from state, use them
      if (stateGroupMembers && stateGroupMembers.length > 0) {
        setGroupMembers(stateGroupMembers);
      }
    }
  }, [user?.id, user?.auth_token, roomId, stateGroupDetails, stateGroupMembers]);
    ...
    return (
      <div className="page-container pt-24">
        <div className="flex items-center gap-4 mb-8">
          <button onClick={() => navigate(-1)} className="text-gray-400 hover:text-white transition-colors p-2 hover:bg-dark-400 rounded-lg">
  ...
  return (
    <div className="page-container pt-24 pb-8">
      ...
      <div className="flex items-center gap-2 md:gap-4 mb-6 md:mb-8">
        <button onClick={() => navigate(-1)} className="text-gray-400 hover:text-white transition-colors p-1.5 md:p-2 hover:bg-dark-400 rounded-lg flex-shrink-0">
          ...
          <div className="p-4 md:p-8 border-b border-gray-700/50">
            ...
            <div className="flex items-start gap-3 mb-4">
              ...
              <div className="text-right flex-shrink-0">
                <div className="text-xl md:text-3xl font-bold text-[#2CFF05] leading-tight">₹{itemPrice}</div>
                <div className="text-[10px] md:text-sm text-gray-400">/device</div>
              </div>
              */}
            </div>
                    ...
                    <div key={member.user_id} className="flex items-center gap-3 bg-dark-400/30 rounded-lg p-3">
                      <div className="relative flex-shrink-0">
                        <div className="w-10 h-10 rounded-full overflow-hidden bg-dark-400 ring-2 ring-gray-700">
                          {member.dp ? (
                            <img src={member.dp} alt={member.fullname} className="w-full h-full object-cover" />
                          ) : (
