...
import FavoriteBrandsSkeleton from '../skeletons/FavoriteBrandsSkeleton';
import CategoriesSkeleton from '../skeletons/CategoriesSkeleton';
import SharedSubscriptionsSkeleton from '../skeletons/SharedSubscriptionsSkeleton';
import RentProductClasses from '../components/RentProductClasses';
import InfiniteScroller from "../components/InfiniteScroller";


import {useCarousels} from '../hooks/carousels/useCarousels';
import {useFavouriteBrands} from '../hooks/brands/useFavouriteBrands';
import {useBrandCategories} from '../hooks/brands/useBrandCategories';
import {usePublicGroups} from '../hooks/groups/usePublicGroups';
  ...
  const handleCarouselClick = (carousel: Carousel) => {
    try {
      ...
      if (data?.args?.brand_id) {
        navigate(`/products/${data.args.brand_id}`);
      } else if (data?.url) {
        ...
        <div className="flex items-center justify-center h-full relative">
          {[...carousels, ...carousels, ...carousels].map((carousel, index) => {
            ...
            return (
              <div
                ...
                style={{
                  transform: `translateX(${offset * 280}px) scale(${
                    isCenter ? 1 : isAdjacent ? 0.75 : Math.abs(offset) === 2 ? 0.5 : 0.25
                  })`,
                }}
                onClick={() => {
                  if (isCenter) {
              ...
              >
                <div className={`aspect-2/1 md:w-96 md:h-48 lg:w-[28rem] lg:h-56 rounded-2xl overflow-hidden shadow-2xl group ${
...
const ExplorePage = () => {
  ...
  return (
    <>
      <div className='pt-[70px] md:pt-[20px]'>
      <InfiniteScroller />
      </div>
              ...
              <div className="flex space-x-4 px-1 ">
                {favouriteBrands.map((brand) => (
                  <div key={brand.brand_id} className="flex flex-col items-center w-20">
                    <Link to={`/products/${brand.brand_id}`} className="group cursor-pointer block">
...
<section className="mb-8 md:mb-12">
  <div className="flex items-center justify-between mb-4 md:mb-6">
    ...
    <Link to="/all-brands" className="text-indigo-400 hover:text-indigo-300 transition-colors">
      {t('common.viewAll')}
    </Link>
  </div>
      ...
      <div className="flex flex-col space-y-6 px-1" style={{ width: `${Math.ceil(brandCategories.length / 2) * (256 + 24)}px` }}>
        ...
        <div className="flex space-x-6">
          {brandCategories.filter((_, index) => index % 2 === 0).map((category) => (
            <div
              key={category.name}
              onClick={() => navigate(`/all-brands?category=${encodeURIComponent(category.name)}`)}
                ...
                <div className="flex flex-col items-end">
                  ...
                  <div className="flex -space-x-1">
                    {category.service_images.slice(0, 4).map((image, index) => (
                      <div
                        key={index}
                        className="w-10 h-10 rounded-full border-2 border-black overflow-hidden bg-white/10 backdrop-blur-sm"
                      >
                        <img
                          src={image}
                          alt=""
...
<section className="mb-8 md:mb-12">
  <div className="flex items-center justify-between mb-4 md:mb-6">
    ...
    <Link to="/public-groups" className="text-blue-400 hover:text-blue-300 transition-colors text-sm md:text-base">
      {t('common.viewAll')}
    </Link>
  </div>
      ...
      <div className="flex flex-col space-y-4 md:space-y-6 px-1" style={{ width: `${Math.ceil(publicGroups.length / 2) * (320 + 24)}px` }}>
        ...
        <div className="flex space-x-4 md:space-x-6">
          {publicGroups.filter((_, index) => index % 2 === 0).map((group) => (
            <Link
              to={`/groups/${group.whatsub_plans.id}?search=${encodeURIComponent(group.name)}`}
              key={group.name}
            ...
            >
              <div className="flex items-start gap-3 md:gap-4 mb-3 md:mb-4">
                <div className="w-14 h-14 md:w-16 md:h-16 rounded-xl overflow-hidden bg-transparent p-1.5 md:p-2 group-hover:bg-white/20 transition-colors flex-shrink-0">
                  <img
                    src={group.room_dp}
                    alt={group.name}
