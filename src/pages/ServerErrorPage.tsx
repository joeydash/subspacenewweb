...
const ServerErrorPage: React.FC<ServerErrorPageProps> = ({ 
  ...
  const stateErrorType = location.state?.errorType || propErrorType || 'unknown';
  const stateMessage = location.state?.message || propMessage;
  const canRetry = location.state?.canRetry !== false;
