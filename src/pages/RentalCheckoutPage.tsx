...
const RentalCheckoutPage = () => {
  ...
  useEffect(() => {
    if (location.state) {
      const { startTime: stateStartTime, endTime: stateEndTime } = location.state as {
        startTime?: string;
        endTime?: string;
      };

      if (stateStartTime) {
        setStartDateTime(new Date(stateStartTime));
      }
      if (stateEndTime) {
        setEndDateTime(new Date(stateEndTime));
      }
    }
  }, [location.state]);
          ...
          <div className="lg:col-span-2 space-y-0">
            <div className="pb-6 border-b border-gray-700/50">
              ...
              <div className="flex items-start gap-3 md:gap-4">
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-xl overflow-hidden bg-white p-2 flex-shrink-0 shadow-lg">
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <div className={`mt-0.5 ${
                          selectedAddressId === address.id ? 'text-blue-400' : 'text-gray-400'
                        }`}>
