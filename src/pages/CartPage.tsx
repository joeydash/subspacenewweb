...
import { useWalletBalance, WALLET_BALANCE_BASE_KEY } from '../hooks/wallet/useWalletBalance';
import { TRANSACTIONS_BASE_KEY } from '../hooks/wallet/useTransactions';
		...
		<div className="max-w-[2000px] px-2 md:px-4 pt-24 md:pt-12 pb-8 md:pb-12">
			...
			{isCartItemsLoading ? (
				...
				<div className="bg-dark-500 rounded-lg p-6 md:p-8 text-center">
					...
					<Link to="/" className="btn btn-primary max-w-[70%] mx-auto">
							...
							{cartData?.items?.map((item, index) => (
								<div key={item.id} className={`relative py-4 ${index !== cartData.items.length - 1 ? 'border-b border-gray-700' : ''}`}>
									<div className="flex gap-3 md:gap-4">
										{/* Product Image */}
										<div className="w-20 h-20 md:w-24 md:h-24 flex-shrink-0 bg-dark-600 rounded-lg overflow-hidden">
											<img
												src={item.image_url}
					...
					<div className="lg:w-96 flex-shrink-0">
						<div className="bg-dark-500 rounded-lg p-4 md:p-6">
							...
							<div className="space-y-3">
								...
								<Link to="/" className="flex items-center justify-center text-gray-400 hover:text-gray-300 text-xs sm:text-sm transition-colors py-1">
