...
const DTHBillPage = () => {
  ...
  const handleRecharge = async () => {
    ...
    if (operatorData?.is_flexipay) {
      ...
      navigate(`/dth-plans?operatorId=${selectedOperator}&customerNumber=${customerNumber}&operatorName=${encodeURIComponent(operatorData?.name || '')}`);
      return;
    }
  ...
  const handleCheckPlans = () => {
    if (customerNumber && selectedOperator) {
      const operatorData = getSelectedOperatorData();
      navigate(`/dth-plans?operatorId=${selectedOperator}&customerNumber=${customerNumber}&operatorName=${encodeURIComponent(operatorData?.Name || '')}&operatorCode=${encodeURIComponent(operatorData?.code || '')}`);
    }
  };
  ...
  return (
    <div className="page-container pt-24">
      ...
      <div className="flex items-center gap-4 mb-8">
        <Link to="/quick-payments" className="text-gray-400 hover:text-white transition-colors p-2 hover:bg-dark-400 rounded-lg">
